...
Each JSON RPC _method_ is implemented as an `EndpointCodec`, a go-base [Endpoint](https://godoc.org/git-codecommit.ap-southeast-1.amazonaws.com/v1/repos/be-base/endpoint#Endpoint), sandwiched between a decoder and encoder. The decoder picks apart the JSON RPC request params, which can be passed to your endpoint. The encoder receives the output from the endpoint and encodes a JSON-RPC result.
